...
#' ## Environmental factors include elevation, season, space (dbMEM1 and dbMEM2), host (em.GR and em.abun), climate (sea.MT), and soil (pH, TP, TK, AN, AP, and AK)
#' env.list <- list(
...
#' upset_vp(baima.env.vp)
#' }

upset_vp <- function(rdacca, plot.hp = TRUE, order.part = 'effect', decreasing.part = TRUE, order.var = TRUE, decreasing.var = TRUE, cutoff = -1, nVar = 30, col.width = 0.6, pch.size = 3, line.lwd = 0.5, show.effect = TRUE, effect.cex = 2.7, title.cex = 10, axis.cex = 8, height.ratio = c(2, 1), width.ratio = c(1, 3)) {
    ...
    if (show.effect)
        p.vp$layers[[2]] <- geom_text(aes(label = Fractions*100, vjust = ifelse(Fractions >= 0, -0.2, 1.2)), color = 'black', size = effect.cex)
    ...
    if (show.effect)
        p.exp$layers[[2]] <- geom_text(aes(label = Fractions*100, hjust = ifelse(Fractions >= 0, 1.2, -0.2)), color = 'black', size = effect.cex)
    ...
    if (plot.hp)
        panel$X1 <- factor(panel$X1, levels = Hier.part$Var)
    else
        panel$X1 <- factor(panel$X1, levels = Var.exp$Var)
    ...
    for (i in levels(panel$X2)) {
        ...
        if (nrow(panel.i) > 1)
            p.panel <- p.panel + annotate('segment', x = i, xend = i, y = panel.i[1,1], yend = panel.i[nrow(panel.i),1], color = 'black', size = line.lwd)
    }
    ...
    if (plot.hp) {
        ...
        if (show.effect)
            p.hp$layers[[2]] <- geom_text(aes(label = Individual*100, hjust = ifelse(Individual >= 0, 1.2, -0.2)), color = 'black', size = effect.cex)
    ...
    if (plot.hp)
        p.vp + p.panel + p.hp + plot_layout(design = design)
    else
        p.vp + p.panel + p.exp + plot_layout(design = design)
}
