...
#' @param col.color Color of bars, default is \code{NULL}, color by grayscale.
...
#' ## Quantify the individual effects of soil properties on the composition of EcM fungal community by using partial dbRDA
...
barplot_hp <- function(rdacca, order.var = TRUE, decreasing.var = TRUE, cutoff = -1, col.fill = 'valid', col.color = NULL, col.width = 0.6, show.effect = TRUE, effect.cex = 2.7, title.cex = 10, axis.cex = 8) {
    ...
    p.hp <- ggplot(Hier.part, aes(Var, Individual*100)) +
        theme(panel.grid = element_blank(),
            ...
            labs(y = 'Individual (%)', x = '', title = paste('Constrained:', Constrained*100, '%', '  ', 'Residual', (1- Constrained)*100, '%'))
    ...
    if (col.fill == 'valid') {
        ...
        p.hp <- p.hp + 
            geom_col(aes(fill = valid), width = col.width) +
        ...
        p.hp <- p.hp +
            geom_col(aes(fill = Var), width = col.width) +
