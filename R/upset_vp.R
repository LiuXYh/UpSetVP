...
upset_vp <- function(rdacca, plot.hp = TRUE, order.part = "effect", decreasing.part = TRUE, order.var = TRUE, decreasing.var = TRUE, cutoff = -1, nVar = 30, col.width = 0.6, pch.size = 3, line.lwd = 0.5, show.effect = TRUE, effect.cex = 2.7, title.cex = 10, axis.cex = 8, height.ratio = c(2, 1), width.ratio = c(1, 3)) {
    ...
    p.vp <- ggplot(data = Var.part, aes_string(x = 'Var', y = 'Fractions', fill = 'valid')) +
    ...
    if (show.effect)
        p.vp$layers[[2]] <- geom_text(aes_string(label = 'Fractions', vjust = ifelse(Var.part$Fractions >= 0, -0.2, 1.2)), color = 'black', size = effect.cex)
    ...
    p.exp <- ggplot(data = Var.exp, aes_string(x = 'Var', y = 'Fractions', fill = 'valid')) +
        geom_col(width = col.width) +
        scale_fill_manual(values = c('gray95', 'black'), limits = c('0', '1')) +
        theme(panel.grid = element_blank(),
            panel.background = element_blank(),
    ...
    if (show.effect)
        p.exp$layers[[2]] <- geom_text(aes_string(label = 'Fractions', hjust = ifelse(Var.exp$Fractions >= 0, 1.2, -0.2)), color = 'black', size = effect.cex)
    ...
    p.panel <- ggplot(data = panel, aes_string(x = 'X2', y = 'X1', color = 'X3', fill = 'X4')) +
        geom_tile(color = NA) +
    ...
    if (plot.hp) {
        ...
        p.hp <- ggplot(data = Hier.part, aes_string(x = 'Var', y = 'Individual', fill = 'valid')) +
            geom_col(width = col.width) +
            scale_fill_manual(values = c('gray95', 'black'), limits = c('0', '1')) +
            theme(panel.grid = element_blank(),
                panel.background = element_blank(),
