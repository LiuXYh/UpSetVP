...
upset_vp <- function(rdacca, plot.hp = TRUE, order.part = "effect", decreasing.part = TRUE, order.var = TRUE, decreasing.var = TRUE, cutoff = -1, nVar = 30, col.width = 0.6, pch.size = 3, line.lwd = 0.5, show.effect = TRUE, effect.cex = 2.7, title.cex = 10, axis.cex = 8, height.ratio = c(2, 1), width.ratio = c(1, 3)) {
    ...
    p.vp <- ggplot(Var.part, aes_string(x = "Var", y = "Fractions", fill = "valid")) +
        ...
        labs(y = "Fractions (%)", x = "", title = paste("Constrained:", Constrained, "%", "  ", "Residual", 100-Constrained, "%"))
    
    if (show.effect)
        p.vp$layers[[2]] <- geom_text(aes_string(label = "Fractions", vjust = ifelse("Fractions" >= 0, -0.2, 1.2)), color = "black", size = effect.cex)
    ...
    Var.exp <- Var.exp[which(Var.exp$Fractions >= cutoff), ]
    Var.exp$Fractions <- 100*Var.exp$Fractions
    if (order.var)
        Var.exp <- Var.exp[order(Var.exp$Fractions, decreasing = !decreasing.var), ]
    Var.exp$Var <- factor(Var.exp$Var, levels = Var.exp$Var)
    
    p.exp <- ggplot(Var.exp, aes_string(x = "Var", y = "Fractions", fill = "valid")) +
        geom_col(width = col.width) +
        scale_fill_manual(values = c("gray95", "black"), limits = c("0", "1")) +
        theme(panel.grid = element_blank(),
            panel.background = element_blank(),
    ...
    if (show.effect)
        p.exp$layers[[2]] <- geom_text(aes_string(label = "Fractions", hjust = ifelse("Fractions" >= 0, 1.2, -0.2)), color = "black", size = effect.cex)
    ...
    if (plot.hp) {
        ...
        p.hp <- ggplot(Hier.part, aes_string(x = "Var", y = "Individual", fill = "valid")) +
            geom_col(width = col.width) +
            scale_fill_manual(values = c("gray95", "black"), limits = c("0", "1")) +
            theme(panel.grid = element_blank(),
                panel.background = element_blank(),
        ...
        if (show.effect)
            p.hp$layers[[2]] <- geom_text(aes_string(label = "Individual", hjust = ifelse("Individual" >= 0, 1.2, -0.2)), color = "black", size = effect.cex)
